...
IF %jver% GTR 18 set TNT4JOPTS=%TNT4JOPTS% "--add-exports=jdk.attach/sun.tools.attach=ALL-UNNAMED" "--add-opens=jdk.attach/sun.tools.attach=ALL-UNNAMED"
...
@echo on
%JAVA_EXEC% %TNT4JOPTS% -classpath "%LIBPATH%" com.jkoolcloud.tnt4j.stream.jmx.SamplingAgent -attach -vm:%1 -ap:%RUNDIR%..\tnt4j-stream-jmx-core-1.18.4-all.jar -ao:%TNT4J_AGENT_OPTIONS% %TNT4J_AGENT_ARGS%