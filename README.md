...
Latest Java 8 compliant [tnt4j-stream-jmx](https://github.com/Nastel/tnt4j-stream-jmx/tree/java8) version is `1.18.x`.
...
**NOTE:** JESL provides a way to stream events generated by Stream-JMX to [meshIQ](https://www.meshiq.com). For more information on
...
* `-ap:tnt4j-stream-jmx-core-all.jar` - is agent library name. If it is class path - then only name should be sufficient. In any other case
  define full or relative path, e.g., `..\build\tnt4j-stream-jmx\tnt4j-stream-jmx-2.2.0\tnt4j-stream-jmx-core-all.jar`. Mandatory argument.
...
1. agent options for MBeans include/exclude filter and sampling interval (optional, `.` value sets default)
1. service identifier for the process/service being monitored  (optional, `.` value sets default)
1. sampling agent arguments (optional, `.` value sets default)
...
* See [Enabling Tomcat JMX Remote](https://tomcat.apache.org/tomcat-7.0-doc/monitoring.html) to enable remote JMX access of Tomcat server
  ...
  on how to set up such Tomcat server instance to be accessible from outside.**
...
1. Use predefined VM descriptor prefixes to initiate VMs JMX connection resolution over running ZooKeeper service instance:
...
* appending `LIBPATH` variable with WebLogic client libs from `${WL_INSTALL_DIR}/wlserver/server/lib/`:
  ...
  For example:
    * .bat/cmd
      ...
      set WL_CLIENT_LIBS=%WL_HOME%\wlserver\server\lib\wlclient.jar;%WL_HOME%\wlserver\server\lib\wljmxclient.jar;%WL_HOME%\wlserver\server\lib\javax.javaee-api.jar
      set LIBPATH=%LIBPATH%;%RUNDIR%..\lib\*;%WL_CLIENT_LIBS%
      ```
    * .sh
      ```bash
      ...
      LIBPATH="$LIBPATH:$SCRIPTPATH/../lib/*:$WL_CLIENT_LIBS"
      ```
...
1. `Troubleshooting` -> `[your server instance]` -> `Change log detail levels` -> select `Configuration`/`Runtime` tab -> into text area
...
<server description="new server">
    ...
    <application contextRoot="tnt-jmx" location="tnt4j-stream-jmx-liberty-war-2.2.0.war" type="war" id="tnt-jmx" name="tnt-jmx">
        <application-bnd>
...
1. Has two dedicated value placeholders `sjmx.serverAddress` and `sjmx.serverName` to resolve remotely sampled machine IP address and host
   ...
   definition. In this case `tnt4j.properties` configuration would be like this:
    ...
    ...
    ```
1. Can resolve values for `source` fields from JMX MBean attributes. Two items are required to use in `tnt4j.properties` configuration to
   enable this feature:
    1. Set `source.factory` to `com.jkoolcloud.tnt4j.stream.jmx.source.JMXSourceFactoryImpl`
    1. Put MBean attribute descriptor having prefix `@bean:` as `source` field value. There are two types of MBean attribute descriptors:
    ...
    1. Since MBean initialization may take some time on server side (usually it may happen after some server component restart) MBean 
...
### To [meshIQ](https://www.meshiq.com): (Requires [JESL libraries](https://nastel.github.io/JESL))
...
In addition to intercepting sample events, applications may want to control how one or more attributes are sampled and whether each sample
...
example, what if you wanted to set up an action when a specific MBean attribute exceeds a certain threshold?
...
* [TNT4J](https://nastel.github.io/TNT4J)
* [meshiq.com](https://www.meshiq.com)
